// ── STORAGE ──
const STORAGE_KEY = 'teksystem_faturas';
const NEXT_ID_KEY = 'teksystem_nextId';

const DEFAULT_FATURAS = [
...
function loadFaturas() {
  try {
    ...
    return stored ? JSON.parse(stored) : DEFAULT_FATURAS;
  } catch {
    return DEFAULT_FATURAS;
  }
}

function saveFaturas() {
  localStorage.setItem(STORAGE_KEY, JSON.stringify(faturas));
}
...
function saveNextId() {
  localStorage.setItem(NEXT_ID_KEY, nextId);
}

// ── DATA ──
let faturas   = loadFaturas();
let editingId = null;
let sortKey   = 'data';
let sortAsc   = false;
